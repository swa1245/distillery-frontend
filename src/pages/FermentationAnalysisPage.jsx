...
import { FileSpreadsheet, FlaskConical, Save, Trash2 } from "lucide-react";
...
const STORAGE_KEY = "distiller_fermentation_table";
...
const STATUS_OPTIONS = ["PF Transfer", "MF Transfer", "Filling", "Setup", "Reaction"];

const PROCESS_COLS = [
  { key: "volPct", label: "Vol (%) of fermentor" },
  { key: "gravity", label: "Gravity" },
  { key: "rsPct", label: "RS (%)" },
  { key: "dstPct", label: "Residual Starch" },
  { key: "ph", label: "pH" },
  { key: "temperatureC", label: "Temperature (°C)" },
];

const HPLC_COLS = [
  { key: "dp4PlusPct", label: "DP4+ (% w/w)" },
  { key: "dp4Pct", label: "DP4 (% w/w)" },
  { key: "dp3Pct", label: "DP3 (% w/w)" },
  { key: "dp2Pct", label: "DP2 (% w/w)" },
  { key: "dp1Pct", label: "DP1 (% w/w)" },
...
const FERM_COLS = [...PROCESS_COLS, ...INPUT_COLS];
const FERM_COL_COUNT = 5 + FERM_COLS.length + 1;
...
function sheetKey(view) {
  return view === "hplc" ? "hplc" : "fermentation";
}
...
const EXPORT_HEADERS = [
  ...
  "Time (h)",
  "Fermentor Number",
  "Status of the fermentor",
  ...FERM_COLS.map((c) => c.label),
];
...
function hydrateRow(row, date, slNo) {
  ...
  return {
    ...
    date: row.date || date,
    time: row.time || "",
    fermenterNo: row.fermenterNo != null ? String(row.fermenterNo) : "",
    status: row.status || "",
  };
}
...
function loadViewStore() {
  try {
    ...
    if (raw?.views && typeof raw.views === "object") {
      return {
        fermentation: raw.views.fermentation || {},
...
export default function FermentationAnalysisPage() {
  const { user } = useAuth();
  const [view, setView] = useState("fermentation");
  ...
  const handleExport = () => {
    const headers = isHplc
      ? ["Sl. No.", "Date", "Time (h)", "Fermentor Number", ...HPLC_COLS.map((c) => c.label)]
      : EXPORT_HEADERS;
    const exportRows = filtered.map((row) =>
      isHplc
        ? [row.slNo, formatDisplayDate(row.date), row.time, row.fermenterNo, ...HPLC_COLS.map((c) => row[c.key])]
        : [
            row.slNo,
  ...
  const addRow = () => {
    ...
    if (typeFilter) next.fermenterNo = typeFilter;
  ...
  const hplcAverages = useMemo(() => {
    const next = {};
    for (const col of HPLC_COLS) next[col.key] = formatAvg(filtered.map((row) => row[col.key]));
    return next;
  }, [filtered]);
  ...
  const dateTimeFermerterCells = (row) => (
    <>
      ...
      <td className="px-1.5 py-1.5 min-w-[138px]">
        <DistillerTimePicker compact value={row.time} onChange={(v) => updateRow(row.id, "time", v)} placeholder="Time" />
      </td>
      <td className="px-1.5 py-1.5 min-w-[110px]">
        <DistillerSelect
          ...
          options={[{ value: "", label: "Select" }, ...FERMENTER_OPTIONS]}
          placeholder="No."
        />
      </td>
    </>
    ...
    <div className="w-full min-h-full px-4 sm:px-6 lg:px-8 py-5">
      <section className="w-full overflow-hidden rounded-2xl border border-sky-200/80 bg-white shadow-[0_8px_28px_rgba(15,39,68,0.08)]">
        ...
        <ReportFilterBar
          ...
          selectLabel="Fermentor"
          selectValue={typeFilter}
          selectOptions={FERMENTER_OPTIONS}
          onSelectChange={setTypeFilter}
          search={search}
          onSearchChange={setSearch}
          searchPlaceholder="Search fermentor, status, time…"
          onAddRow={addRow}
        />

        {isHplc ? (
          <div className="overflow-x-auto">
            <table className="min-w-[1600px] w-full text-left text-[11px] border-collapse">
              <thead>
                <tr className="bg-[#0f2744] text-white font-extrabold uppercase tracking-wider text-[10px]">
                  ...
                  <th rowSpan={2} className="px-3 py-2.5 border-r border-white/10 min-w-[138px]">
                    Time (h)
                  </th>
            ...
            <table className="min-w-[1680px] w-full text-left text-[11px] border-collapse">
              <thead>
                <tr className="bg-[#0f2744] text-white font-extrabold uppercase tracking-wider text-[10px]">
                  ...
                  <th rowSpan={2} className="px-2 py-2.5 border-r border-white/10 min-w-[140px] text-center">
                    Status of the fermentor
                  </th>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={FERM_COL_COUNT} className="px-4 py-12 text-center text-sm font-bold text-stone-400">
                      No readings yet. Click Add row.
                    </td>
                  </tr>
                ) : (
                      ...
                      <tr key={row.id} className="bg-white align-top">
                        ...
                        <td className="px-1.5 py-1.5 min-w-[140px]">
                          <DistillerSelect
                            ...
                            options={[{ value: "", label: "Select" }, ...STATUS_OPTIONS]}
                            placeholder="Status"
                          />
                        </td>
